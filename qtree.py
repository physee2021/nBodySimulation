...
class Rect:
    ...
    def draw(self, ax=None, c='k', lw=1, **kwargs):
        x1, y1 = self.west_edge, self.north_edge
        x2, y2 = self.east_edge, self.south_edge
        pygame.draw.rect(screen, (0,0,0), (x1, y1, x1 - x2, y1 - y2), 1)
...
class QuadTree:
    ...
    # find and return how many particles are in each node. 
    def findNodes(self, node, points):
        try:
            if node.divided:
                node.findNodes(node.ne, points)
                node.findNodes(node.nw, points)
                node.findNodes(node.se, points)
                node.findNodes(node.sw, points)
                
                # return[node.query(node.ne.boundary, []),
                # node.query(node.nw.boundary, []),
                # node.query(node.se.boundary, []),
                # node.query(node.sw.boundary, [])]
            else:
                
                points.append(node.query(node.boundary, []))
